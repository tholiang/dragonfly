...
int DragonflyUtils::FaceIntercept(Model *m, vec_float3 start, vec_float3 vector, int avoid) {
    int fid = -1;
    float minz = -1;
    ...
    for (int i = 0; i < m->NumFaces(); i++) {
        if (i != avoid) {
            ...
            if (t > 0 && t < 10000) {
                vec_float3 p = vec_make_float3(start.x + t * vector.x, start.y + t * vector.y, start.z + t * vector.z);
...
//    std::cout<<radius<<std::endl;
    ...
    vec_float3 center = TriAvg(m->GetVertex(currface->vertices[0]), m->GetVertex(currface->vertices[1]), m->GetVertex(currface->vertices[2]));
    vec_float3 vector = GetNormal(m->GetVertex(currface->vertices[0]), m->GetVertex(currface->vertices[1]), m->GetVertex(currface->vertices[2]));
...
//        std::cout<<std::endl;
        ...
        if (nextfid != -1) {
            ...
            vec_float3 normal = GetNormal(m->GetVertex(currface->vertices[0]), m->GetVertex(currface->vertices[1]), m->GetVertex(currface->vertices[2]));
            ...
            vec_float3 newend = RotateAround(vecend, center, vec_make_float3((float(rand() % 1600) / 1000) - 0.8, (float(rand() % 1600) / 1000) - 0.8, (float(rand() % 1600) / 1000) - 0.8));
            ...
            vec_float3 distpoly = DistancePolynomial(center, vector, origin);
            ...
            vec_float3 edgevec = vec_make_float3(origin.x-center.x, origin.y-center.y, origin.z-center.z);
            float mag = dist3to3(edgevec, vec_make_float3(0, 0, 0));
            edgevec.x /= mag;
            edgevec.y /= mag;
            edgevec.z /= mag;
            
            vec_float3 vecend = vec_make_float3(center.x + edgevec.x, center.y + edgevec.y, center.z + edgevec.z);
            
            // rotate within 45 degrees randomly
            vec_float3 newend = RotateAround(vecend, center, vec_make_float3((float(rand() % 800) / 1000) - 0.4, (float(rand() % 800) / 1000) - 0.4, (float(rand() % 800) / 1000) - 0.4));
