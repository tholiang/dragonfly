...
DragonflyUtils::PointData *DragonflyUtils::PointDataFromFile(std::string path) {
    ...
    if (myfile.is_open()) {
        ...
        for (int i = 0; i < num_points; i++) {
            ...
            pd->points.push_back(vec_make_float2(x, y));
...
std::vector<vec_int3> DragonflyUtils::FindPointDataTriangles(PointData* pd) {
    std::vector<vec_int3> triangles;
            ...
            for (int k = 0; k < pd->edges[p2].size(); k++) {
                ...
                for (int l = 0; l < pd->edges[p3].size(); l++) {
                    ...
                    if (p4 == p1 && p2 > p1 && p3 > p2) { // last two checks to avoid repeats
                        ...
                        if (valid) {
                            triangles.push_back(vec_make_int3(p1, p2, p3));
                        }
                    }
                }
            }
...
Model * DragonflyUtils::ModelFromPointData(PointData *pd) {
    std::vector<vec_int3> triangles = FindPointDataTriangles(pd);
    
    Model *m = new Model();
    for (int i = 0; i < pd->points.size(); i++) {
        vec_float2 p = pd->points[i];
        m->MakeVertex(p.x, 0, p.y);
    }
    
    for (int i = 0; i < triangles.size(); i++) {
        vec_int3 triangle = triangles[i];
