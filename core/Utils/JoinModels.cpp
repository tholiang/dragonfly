...
std::vector<int> DragonflyUtils::Hungarian(std::vector<vec_float3> &A, std::vector<vec_float3> &B) {
...
void DragonflyUtils::JoinModels(Model *A, Model *B, ModelTransform *muA, ModelTransform *muB, std::vector<int> &A_pts, std::vector<int> &B_pts) {
    std::vector<vec_float3> A_vals;
    std::vector<vec_float3> B_vals;
    ...
    vec_float3 modeldiff = vec_make_float3(muB->b.pos.x - muA->b.pos.x, muB->b.pos.y - muA->b.pos.y, muB->b.pos.z - muA->b.pos.z);
...
void DragonflyUtils::CapModel(Model *m, std::vector<int> vertices) {
    Vertex avg = vec_make_float3(0, 0, 0);
    ...
    for (int i = 0; i < vertices.size(); i++) {
        ...
        for (int j = 0; j < neighbors.size(); j++) {
            ...
            if (!prevlinked) {
                m->MakeFace(vid, neighbor, newid, vec_make_float4(1, 1, 1, 1));
...
void DragonflyUtils::BridgeEqualModels(Model *a, ModelTransform *au, std::vector<int> &avertices, Model *b, ModelTransform *bu, std::vector<int> &bvertices) {
    ...
    while (true) {
        ...
        a->MakeFace(avid1, avid2, bvid1+bvertexstart, vec_make_float4(1, 1, 1, 1));
        a->MakeFace(avid2, bvid1+bvertexstart, bvid2+bvertexstart, vec_make_float4(1, 1, 1, 1));
